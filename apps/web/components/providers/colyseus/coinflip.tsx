...
function CoinFlipRoomMessageHandler() {
...
export function CoinFlipRoomProvider({ children }: { children: ReactNode }) {
    const { hexAddress: userWalletAddress } = useInterwovenKit()
    const { data } = useSWR<SeatReservation>('/api/user', () => gameClient.http.post("/api/join-room", { body: { user: userWalletAddress ?? "Anonymous", room: "coinflip" } }).then(e => e.data).catch(() => null))
    return (
        <RoomProvider connect={data ? () => gameClient.consumeSeatReservation(data) : false} >
            <CoinFlipRoomStatus />
            <CoinFlipRoomMessageHandler />
            {children}
        </RoomProvider>
    )
}
