...
// Pre-made textures for teams 0-3; teams 4-7 reuse base + tint
const TEAM_TEXTURES_PATHS = [
  "/battle-of-tanks/models/T_pixelTank_red.png",
  "/battle-of-tanks/models/T_pixelTank_blue.png",
  "/battle-of-tanks/models/T_pixelTank_green.png",
  "/battle-of-tanks/models/T_pixelTank_yellow.png",
];
const BASE_TEXTURE_PATH = "/battle-of-tanks/models/T_pixelTank.png";

// Tint colors multiplied with the base texture for teams 4-7
const TEAM_TINTS: (number | null)[] = [
  null, // 0 — use dedicated texture
  null, // 1
  null, // 2
  null, // 3
];
...
export function preloadTankModel(): Promise<THREE.Group> {
  ...
  modelLoadPromise = new Promise((resolve, reject) => {
    ...
    // Load dedicated team textures (0-3)
    ...
    // Load base texture for tinted teams (4-7)
    baseTexture = texLoader.load(BASE_TEXTURE_PATH);
    baseTexture.magFilter = THREE.NearestFilter;
    baseTexture.minFilter = THREE.NearestFilter;
    baseTexture.colorSpace = THREE.SRGBColorSpace;

    loader.load(
      "/battle-of-tanks/models/pixelTank.fbx",
      (fbx) => {
...
function makeTeamMaterial(team: number): THREE.MeshStandardMaterial {
  ...
  if (tint !== null && tint !== undefined) {
    // Tinted team: base texture × tint color
...
function createCrownTexture(): THREE.Texture {
  ...
  ctx.beginPath(); ctx.arc(58, 20, 3, 0, Math.PI * 2); ctx.fill();
...
let crownTexture: THREE.Texture | null = null;

function getCrownTexture(): THREE.Texture {
  if (!crownTexture) crownTexture = createCrownTexture();
  return crownTexture;
}
...
export class TankEntity {
  ...
  shieldBg: THREE.Sprite;
  nameLabel: THREE.Sprite;
  leaderCrown: THREE.Sprite;
  ...
  constructor(team: number) {
    ...
    this.healthBg.scale.set(1.5, 0.12, 1);
    ...
    this.healthBar.scale.set(1.5, 0.12, 1);
    this.healthBar.position.y = 2.2;
    this.group.add(this.healthBar);

    // Shield bar (above health)
    const sbBgMat = new THREE.SpriteMaterial({
      color: 0x000000,
      transparent: true,
      opacity: 0.5,
    });
    this.shieldBg = new THREE.Sprite(sbBgMat);
    this.shieldBg.scale.set(1.5, 0.1, 1);
    this.shieldBg.position.y = 2.35;
    this.group.add(this.shieldBg);

    const sbMat = new THREE.SpriteMaterial({ color: 0x44ccff });
    this.shieldBar = new THREE.Sprite(sbMat);
    this.shieldBar.scale.set(0, 0.1, 1);
    this.shieldBar.position.y = 2.35;
    this.group.add(this.shieldBar);

    // Initial state: hide bars if full or empty
    this.shieldBg.visible = false;
    ...
    this.nameLabel.position.y = 2.7; // above health bar
    ...
    // Leader crown
    this.leaderCrown = new THREE.Sprite(new THREE.SpriteMaterial({
      map: getCrownTexture(),
      transparent: true,
      depthTest: false,
    }));
    this.leaderCrown.scale.set(0.65, 0.65, 1);
    this.leaderCrown.position.y = 3.2;
    this.leaderCrown.visible = false;
    this.group.add(this.leaderCrown);
  }
  ...
  update() {
    ...
    this.shieldBar.visible = tankVisible && this.shieldActive;
    this.shieldBg.visible = tankVisible && this.shieldActive;
    this.nameLabel.visible = tankVisible;
    this.leaderCrown.visible = tankVisible && this.isLeader;

    // Crown animation
    if (this.isLeader) {
      this.leaderCrown.position.y = 3.2 + Math.sin(Date.now() * 0.005) * 0.15;
      this.leaderCrown.rotation.z = Math.sin(Date.now() * 0.002) * 0.1;
    }

    // Speed boost visual
    if (this.speedEnd > Date.now()) {
      // Pulse the team indicator yellow if speed is active
      const pulse = 0.5 + Math.sin(Date.now() * 0.015) * 0.5;
      (this.teamIndicator.material as THREE.MeshBasicMaterial).color.setRGB(1, 1, 0.2 + pulse * 0.8);
      this.teamIndicator.scale.setScalar(1.0 + pulse * 0.1);
    } else {
      // Reset color
      (this.teamIndicator.material as THREE.MeshBasicMaterial).color.setHex(TEAM_COLORS[this.team] || 0xffffff);
      this.teamIndicator.scale.setScalar(1.0);
    }
  }

  setSpeedBoost(endTimestamp: number) {
    this.speedEnd = endTimestamp;
  }
  ...
  setShield(val: number) {
    ...
    // Update floating shield bar
    const pct = Math.max(0, val / 10);
    this.shieldBar.scale.set(1.5 * pct, 0.1, 1);
    this.shieldBar.position.x = -(1.5 * (1 - pct)) / 2;
    this.shieldBar.visible = this.shieldActive;
    this.shieldBg.visible = this.shieldActive;
  ...
  setHealth(hp: number) {
    ...
    this.healthBar.scale.set(1.5 * pct, 0.12, 1);
    this.healthBar.position.x = -(1.5 * (1 - pct)) / 2;
