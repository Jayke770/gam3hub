export class Sound {
  ...
  private destroyedBuffer: AudioBuffer | null = null;
  ...
  constructor() {
    ...
    this.loadSound("/battle-of-tanks/sounds/destroyed.wav")
      .then((buf) => (this.destroyedBuffer = buf))
      .catch((err) => console.error("Failed to load destroyed sound", err));
  }
  ...
  destroyed() {
    // Play the procedurally generated explosion (old sound)
    this.explosion();

    // Play the new wav sound if available
    if (this.destroyedBuffer) {
      const source = this.ctx.createBufferSource();
      source.buffer = this.destroyedBuffer;
  ...
  pickupSpeed() {
    // Fast rising "zipping" sound
    this.tone(0.1, 400, 1000, 0.15);
    setTimeout(() => this.tone(0.1, 600, 1200, 0.12), 40);
    setTimeout(() => this.tone(0.1, 800, 1400, 0.1), 80);
  }
