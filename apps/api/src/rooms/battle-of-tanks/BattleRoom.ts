...
const PICKABLE_SPAWNS = [
  { x: 40, y: 15, type: "repair", delay: 5000 },
  { x: 65, y: 40, type: "repair", delay: 5000 },
  { x: 40, y: 65, type: "repair", delay: 5000 },
  { x: 15, y: 40, type: "repair", delay: 5000 },
  { x: 22, y: 22, type: "damage", delay: 10000 },
  { x: 58, y: 22, type: "damage", delay: 10000 },
...
export class BattleRoom extends Room {
  ...
  onCreate() {
    ...
    // Blocks
    for (const [bx, by, bw, bh, btype] of LEVEL) {
      const block = new Block(bx, by, bw, bh, btype || 0);
  ...
  private update() {
    ...
    for (const block of this.blocks) {
      if (block.type !== 0) {
        block.update(1000 / 20);
        this.world.updateItem("block", block);
      }
    }
